...
import { INITIAL_COURSES as baseCourses, INITIAL_CATEGORIES as categories } from '../backend-data';
...
const Reports: React.FC = () => {
  ...
  const refreshData = async () => {
    const [u, c, e, m] = await Promise.all([
      TallmanAPI.getUsers(),
                ...
                return (
                  <tr key={user.user_id} className="hover:bg-slate-50 transition-colors group print:break-inside-avoid">
                    <td className="px-10 py-8 print:px-0 print:py-4">
                      <div className="flex items-center gap-4">
                        <img src={user.avatar_url} className="w-10 h-10 rounded-xl object-cover border shadow-sm print:hidden" alt="" />
                        <div>
