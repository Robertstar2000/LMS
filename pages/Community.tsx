...
import { INITIAL_FORUM_POSTS as initialPosts } from '../backend-data';
              ...
              <div className="space-y-2">
                 {channels.map(c => (
                   <button 
                     ...
                     className={`w-full text-left px-5 py-3 rounded-xl text-sm font-black transition-all flex items-center justify-between group ${
                       activeChannel === c ? 'bg-indigo-600 text-white shadow-lg translate-x-1' : 'text-slate-500 hover:bg-slate-50'
                     }`}
                   >
                      <span># {c}</span>
           ...
           {filteredPosts.length === 0 ? (
             ...
             <div key={post.id} className={`bg-white p-10 rounded-[3rem] border-2 hover:border-indigo-600 transition-all shadow-sm ${post.is_pinned ? 'ring-4 ring-indigo-50 border-indigo-100' : 'border-slate-50'}`}>
                <div className="flex gap-6 items-start mb-6">
                   <img src={post.author_avatar} className="w-16 h-16 rounded-2xl object-cover shadow-md" alt="" />
